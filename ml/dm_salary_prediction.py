...
def save_local_and_hdfs(df_save, filename):
    """Lưu CSV local rồi put lên HDFS bằng subprocess — không dùng
    spark.createDataFrame() để tránh lỗi pickle khi serialize closures
    còn sót lại trong scope (lr, rf, gbt, evaluator, ...)."""
...
# Lưu bảng so sánh (local + HDFS, không qua Spark)
save_local_and_hdfs(df_results, "job3_model_comparison.csv")

# Lưu predictions của GBT
pred_local_path = "job3_gbt_predictions.csv"
pred_gbt_pd = pred_gbt.select(
...
save_local_and_hdfs(pred_gbt_pd, pred_local_path)
